...
|                    | A revnet can accept and hold any number of tokens, though it defines its price in one consistant currency. If it issues 1,000 tokens per ETH and the revnet is paid some USDC, tokens are issued given the current USDC/ETH price. The revnet can be set up to either hold the USDC received and allow its token holders to cash out from this balance, or auto swap the USDC for an asset it prefers to hold. The network's tokens are backed by the tokens it chooses to hold, inheriting their risks and opportunities.                                                       
                                                                                    |
